...
export function FigmaBrandScanPanel() {
    ...
    const handleFetchFrames = useCallback(async () => {
        ...
        if (!target) {
            setError("Paste a valid Figma file or project URL");
            return;
        }

        setError("");
        setScanState("loading-frames");
        setScanResult(null);
        try {
            if (target.type === "project") {
                const projRes = await figmaApi.projectFiles(target.id);
                if (!projRes.files?.length) throw new Error("No files found in this project");

                let allFrames: FigmaFrame[] = [];
                for (const f of projRes.files) {
                    try {
                        const res = await figmaApi.fileFrames(f.key);
                        if (res.frames?.length) allFrames = [...allFrames, ...res.frames];
                    } catch {
                        /* skip */
                    }
                }
                if (!allFrames.length) throw new Error("Could not find supported design frames");
                setFrames(allFrames);
    ...
    const handleScan = useCallback(async () => {
        if (!frames.length || !selected.size) return;
        setScanState("scanning");
        setError("");
        try {
            const selectedFrames = frames.filter((f) => selected.has(`${f.file_key}:${f.id}`));
            const byFile = selectedFrames.reduce(
                (acc, f) => {
            ...
            const results = await Promise.all(
                Object.keys(byFile).map((k) => figmaApi.batchScan(k, byFile[k]))
            );
            ...
            const totalScoreSum = results.reduce(
                (acc, r) => acc + r.summary.average_score * r.summary.scanned,
                0
            );
            mergedResult.summary.average_score =
                mergedResult.summary.scanned > 0
    ...
    const toggleSelect = (compKey: string) => {
        setSelected((prev) => {
            ...
            if (n.has(compKey)) n.delete(compKey);
            else n.add(compKey);
            return n;
        });
    };
    ...
    const toggleAll = () => {
        setSelected((prev) =>
            prev.size === frames.length ? new Set() : new Set(frames.map((f) => `${f.file_key}:${f.id}`))
        );
    };
    ...
    return (
        <>
                ...
                <div className="panel" style={{ marginBottom: "var(--s-4)", width: "100%" }}>
                    <div className="panel__header">
                        ...
                        <span className="badge badge--live">
                            {figmaStatus?.user?.handle
                                ? `Connected · ${figmaStatus.user.handle}`
                                : "Connected"}
                        </span>
                    </div>
                    <div className="panel__body">
                        <div style={{ display: "flex", gap: "var(--s-2)" }}>
                            <input
                                ...
                                onKeyDown={(e) => e.key === "Enter" && void handleFetchFrames()}
                                style={{ flex: 1 }}
                            />
                            <button
                                type="button"
                                className="btn btn--primary"
                                onClick={() => void handleFetchFrames()}
            ...
            {showInitialEmpty && (
                <div className="page-empty-shell page-empty-shell--section" style={{ width: "100%" }}>
            ...
            {frames.length > 0 && (
                <div className="panel" style={{ marginBottom: "var(--s-4)" }}>
                    ...
                    >
                        <span className="panel__title">
                            <BrushOutlinedIcon sx={{ fontSize: 16 }} /> Design Assets ({frames.length})
                        </span>
                        <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                            ...
                            >
                                {scanState === "scanning"
                                    ? `Scanning ${selected.size} assets...`
                                    : `Scan ${selected.size} Selected`}
                            </button>
                        </div>
                    </div>
                    <div
                    ...
                    >
                        {frames.map((f) => {
                            ...
                            const result = scanResult?.results.find(
                                (r) =>
                                    r.node_id === f.id &&
                                    frames.find((fr) => fr.id === r.node_id)?.file_key === f.file_key
                            );
                            return (
                                <div
                                    key={compKey}
                                    onClick={() => toggleSelect(compKey)}
                                    style={{
                                ...
                                >
                                    ...
                                    <div style={{ padding: "8px 10px", borderTop: "1px solid var(--c-border)" }}>
                                        ...
                                        <div style={{ fontSize: "var(--fs-10)", color: "var(--c-text-muted)" }}>
                                            {f.page} · {f.type}
                                        </div>
                                    </div>
                                        ...
                                        {r.checks
                                            ...
                                            .map((c) => (
                                                ...
                                                >
                                                    {c.status === "pass" ? (
                                                        <CheckCircleOutlinedIcon
                                                            sx={{ fontSize: 14, color: "#34d399", mt: "2px" }}
                                                        />
                                                    ) : c.status === "fail" ? (
                                                        <ErrorOutlineOutlinedIcon
                                                            sx={{ fontSize: 14, color: "#f87171", mt: "2px" }}
                                                        />
                                                    ) : (
                                                        <WarningAmberOutlinedIcon
                                                            sx={{ fontSize: 14, color: "#fbbf24", mt: "2px" }}
                                                        />
                                                    )}
                                                    <div>
            ...
            {error && (
                <div className="alert alert--danger" style={{ marginTop: "var(--s-4)" }}>
                    {error}
                </div>
            )}
        </>
